...
#pragma warning restore IDE1006 // Naming Styles
        ...
        public DelegateCommand LoadPresetToSlotCommand { get; private set; }
        ...
        public MainWindowViewModel(IEventAggregator ea) {
            ...
            try {
                ...
                // Set window title.
                WindowTitle = $"{WindowTitle} {CurrentVersion.VersionString()}";
            }
            catch (Exception ex) {
            ...
            try {
                ...
                LoadPresetToSlotCommand = new DelegateCommand(LoadPresetToSlot);
        ...
        private void GetOffsets() {
            ...
            var gameDirectory = new DirectoryInfo(MemoryService.Process.MainModule.FileName);
            ...
            //var MemoryService = new MemoryService(GameProcess.BaseProcess);

            try {
                //logger.Info($"Could not get offset.json, using AoB to get offset....");
                Offsets = new Offsets(MemoryService);

                //logger.Info($"WayMarkOffset {WayMarkOffset:X}");
            }
            catch (Exception) {
                MessageBox.Show("Could Get Offset. Shutting down.", "Paisley Park", MessageBoxButton.OK, MessageBoxImage.Error);
                Application.Current.Shutdown();
            }
        ...
        private bool InitializeNhaama() {
            ...
            if (procs.Length > 1 || procs.Length == 0) {
                ...
                MemoryService = new MemoryService(procs[0]);
            ...
            MemoryService.Process.EnableRaisingEvents = true;

            // Listen to some stuff.
            MemoryService.Process.Exited += (_, e) => {
                MessageBox.Show(
            ...
            try {
                ...
                ffxiv_folder = Path.GetDirectoryName(MemoryService.Process.MainModule.FileName);
        ...
        private bool ShowProcessSelector(Process[] procs) {
            ...
            MemoryService = new MemoryService(vm.SelectedProcess);
        ...
        */
        ...
        private void OnWork(object sender, DoWorkEventArgs e) {
            ...
            while (true) {
                ...
                Waymark ReadWaymark(IntPtr addr, WaymarkID id) => new Waymark {
                    X = MemoryService.Read<float>(addr),
                    Y = MemoryService.Read<float>(addr + 0x4),
                    Z = MemoryService.Read<float>(addr + 0x8),
                    Active = MemoryService.Read<Byte>(addr + 0x1C) == 1,
                    ID = id
                };

                try {
                    // Read waymarks in with our function.
                    GameMemory.MapID = MemoryService.Read<UInt16>(mapID);
        ...
        public void WriteWaymark(Waymark waymark, int id = -1) {
            ...
            if (UserSettings.LocalOnly) {
                ...
                MemoryService.Write(markAddr, waymark.X);
                MemoryService.Write(markAddr + 0x4, waymark.Y);
                MemoryService.Write(markAddr + 0x8, waymark.Z);

                MemoryService.Write(markAddr + 0x10, (int)(waymark.X * 1000));
                MemoryService.Write(markAddr + 0x14, (int)(waymark.Y * 1000));
                MemoryService.Write(markAddr + 0x18, (int)(waymark.Z * 1000));

                // Write the active state
                MemoryService.Write(markAddr + 0x1C, (byte)(waymark.Active ? 1 : 0));
        ...
        private void LoadPresetToSlot() {
            ...
            try {
                ...
                WaymarkThread = new Thread(() => {
                    ...
                    MemoryService.Write(SlotOffset, importdata);
                });
